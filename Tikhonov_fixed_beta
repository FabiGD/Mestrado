...
class OPENBF_InverseProblem:
    ...
    def A_matrix(self, ID, beta_opt, vessel, knumber):
        ...
        if os.path.exists(file_path):
            ...
            # Saves A matrix 
            output_dir = os.path.join(openBF_dir, f"A_matrix_ID={ID}_beta={beta_opt}")
    ...
    def B_matrix(self, ID, beta_opt, vessel, knumber):
        ...
        B_file = os.path.join(B_dir, f"B_matrix_{vessel}_ID={ID}_beta={beta_opt}.last")
    ...
    def optimized_parameters(self, ID, vessel, alpha, beta_opt, knumber):
        ...
        A_matrix_path = os.path.join(openBF_dir, f"A_matrix_ID={ID}_beta={beta_opt}", f"A_matrix_{vessel}.txt")
        ...
        B_matrix_path = os.path.join(openBF_dir, "B_matrix", f"B_matrix_{vessel}_ID={ID}_beta={beta_opt}.last")
        ...
        print_path = os.path.join(openBF_dir, f"A_matrix_ID={ID}_beta={beta_opt}", f"condition_{vessel}_k{knumber}.txt")
    ...
    def plot_error(self, ID, fixed_beta, vessel, knumber_max):
        ...
        # Plots directory
        plots_dir = os.path.join(openBF_dir, f"iteration_plots_ID={ID}_beta={fixed_beta}")
        os.makedirs(plots_dir, exist_ok=True)
    ...
    def plot_iter(self, ID, fixed_beta, vessel, delta_dict, knumber_max: int):
    ...
    def iteration(self, ID, fixed_beta, knumber, vessel, alpha, add_h0, add_L, add_R0, add_Rp, add_Rd, add_E):
    ...
    def search_opt(self, ID, fixed_beta, vessel, alpha, add_h0, add_L, add_R0, add_Rp, add_Rd, add_E, knumber_max):
        ...
        for knumber in range(0, knumber_max + 1):
            self.iteration(ID, fixed_beta, knumber, vessel, alpha, add_h0, add_L, add_R0, add_Rp, add_Rd, add_E)

        # Plots RMSE for k from 0 to knumber_max
        self.plot_error(ID, fixed_beta, vessel, knumber_max)

        # Plots the parameters for each iteration
        self.plot_iter(ID, fixed_beta, vessel, add_values, knumber_max)
    ...
    def L_curve(self, ID, vessel, knumber, plot=True):
        ...
        if plot:
            ...
            # Plot directory
            plot_dir = os.path.join(openBF_dir, f"iteration_plots_Lcurve_ID={ID}")
    ...
    def Morozov(self, ID, vessel, knumber, plot=True):
        ...
        if plot:
            ...
            # Plot directory
            plot_dir = os.path.join(openBF_dir, f"iteration_plots_discrepancy_ID={ID}")
            os.makedirs(plot_dir, exist_ok=True)
            ...
            plt.xlabel('β')
            plt.ylabel('Discrepancy:' + r'$ [\|z-h(\theta_0)-J_k(\theta-\theta_0)/\sigma\|_2^2 - \delta_G^2]$')
...
if __name__ == "__main__":
    ...
    exponents = np.arange(-8, 3, 0.5)  # 3 is exclusive, so it goes up to 2
    beta_values = 10.0 ** exponents
    for fixed_beta in beta_values:
        updater.search_opt(20, fixed_beta, "vessel1", 0.3, 0.00001, 0.001, 0.0001, 0, 0, 0, 40)
